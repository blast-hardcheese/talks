...
object AkkaHttpImplicits {
  ...
  implicit final def jsonMarshaller(implicit printer: _root_.io.circe.Printer = _root_.io.circe.Printer.noSpaces): ToEntityMarshaller[io.circe.Json] = Marshaller.withFixedContentType(MediaTypes.`application/json`) {
    json => HttpEntity(MediaTypes.`application/json`, printer.print(json))
  }
  implicit final def jsonEntityMarshaller[A](implicit J: io.circe.Encoder[A], printer: _root_.io.circe.Printer = _root_.io.circe.Printer.noSpaces): ToEntityMarshaller[A] = jsonMarshaller(printer).compose(J.apply)
    ...
    (httpEntity, byteString) => if (byteString.isEmpty) {
      ...
      val parseResult = Unmarshaller.bestUnmarshallingCharsetFor(httpEntity) match {
        case HttpCharsets.`UTF-8` =>
          _root_.io.circe.jawn.parse(byteString.utf8String)
        case otherCharset =>
          _root_.io.circe.jawn.parse(byteString.decodeString(otherCharset.nioCharset.name))
      }
      parseResult.fold(FastFuture.failed, FastFuture.successful)
    }
  }
  final val stringyJsonUnmarshaller: FromStringUnmarshaller[io.circe.Json] = Unmarshaller.strict(value => _root_.io.circe.Json.fromString(value))
